...
% The code is iterated to calculate orbital elements of trajectory between
% two points in space and within given amount of time. Using Lambert's
% problem, we can estimate the orbital parameters:
%
% Tabulated Variables:
%
...
delta_t     = input('Enter elapsed time between successive vectors (sec): ');
...
% Estimating F(z) and F'(z) to predict the trajectory:
z       = (-10:0.1:10);
s_z     = [];
c_z     = [];
...
fprintf('\n\nObserve the graph and enter the value of z nearest to the point')
fprintf('\nwhere F(z) changes its sign i.e. goes from positive to negative') 
fprintf('\nor vice versa. There is no need for the value to be accurate, just')
fprintf('\nshould be close to inflection point. \n\n')
...
% Estimating orbital elements for the orbit:
V1_M    = norm(V1_V);
...
if e_m == 0
    fprintf('\n\n The orbit is circular in nature.')
elseif e_m == 1
    fprintf('\n\n The orbit is parabolic in nature.')
elseif e_m > 1
    fprintf('\n\n The orbit is hyperbolic in nature.')
else 
    fprintf('\n\n The orbit is elliptic in nature.')
end
